...
document.addEventListener('DOMContentLoaded', function() {
    ...
    purchaseForm.addEventListener('submit', function(e) {
        ...
        if (!fullName || !phone || !email || !address || !product) {
            e.preventDefault();
...
function doGet(e) {
  try {
    ...
    // Trả về kết quả dưới dạng JSONP
    return ContentService.createTextOutput(e.parameter.callback + '({"result":"success"})')
      .setMimeType(ContentService.MimeType.JAVASCRIPT);
  } catch (error) {
    // Trả về lỗi nếu có
    return ContentService.createTextOutput(e.parameter.callback + '({"result":"error","message":"' + error.toString() + '"})')
      .setMimeType(ContentService.MimeType.JAVASCRIPT);
  }
}
...
function doPost(e) {
  try {
    ...
    return HtmlService.createHtmlOutputFromFile('thank-you')
      .setTitle('Cảm ơn bạn - Julie Lipstick');
  } catch (error) {
    // Trả về trang lỗi
    return HtmlService.createHtmlOutputFromFile('error')
      .setTitle('Lỗi - Julie Lipstick');
  }
} 